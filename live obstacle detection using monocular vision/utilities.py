...
def draw_matches(img1, kp1, img2, kp2, matches):
    ...
    # (a.k.a) a montage
    rows1 = img1.shape[0]
    columns1 = img1.shape[1]
    rows2 = img2.shape[0]
    columns2 = img2.shape[1]

    out = np.zeros((max([rows1, rows2]), columns1 + columns2, 3), dtype='uint8')

    # Place the first image to the left
    out[:rows1, :columns1] = np.dstack([img1, img1, img1])

    # Place the next image to the right of it
    out[:rows2, columns1:] = np.dstack([img2, img2, img2])
    ...
    for match in matches:
        ...
        cv2.circle(out, (int(x2) + columns1, int(y2)), 4, (255, 0, 0), 1)

        # Draw a line in between the two points
        # thickness = 1
        # colour blue
        cv2.line(out, (int(x1), int(y1)), (int(x2) + columns1, int(y2)), (255, 0, 0), 1)

    # if want: retruning the images
    return out
