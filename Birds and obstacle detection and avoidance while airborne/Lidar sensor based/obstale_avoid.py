...
# import py scripts
import sim
from flight_data import FlghtData
from basic_cmnds import FlightCommands
from disable_program import stoppableThread
from sensor import Sensor
from vehicle_main import Vehicle

#creating the class for avoiding the obstacles:
class ObstacleAvoidance(stoppableThread):
    ...
    # vehicle object is an object from external module that has been developed by a different person within the company.
    # Since this module use other modules which is not related
    ...
    def __init__(self):
        ...
        # always keep track on drones last move
        ...
        self.__flight_data = FlghtData() #flightdata(vehicle)

        # Counter and flag for being aware of a sequence of illegal sensor inputs in order to be aware of a malfunction in the system
        ...
        if self.__sensors.connect() == -1:
            raise ConnectionError('Can not connect to the UAV')
        
        print('Connected Successfully to Sensors!')
    ...
    def run(self):
        while not self.stopped():
            sim.altitude_change()   # Delete when moving to a full functioning system.
            ...
            if ahead_distance <= self.DANGER_DISTANCE:
                ...
                # In such case the algorithm would slow down the drone drastically in order to give it more time to manouver and avoid a collision.
                print("CAUTION: Obstacle in less than 10 meters!")
            ...
            # If already tried going to go to the left/right 7 meters 
            # There'sstill an obstacle ahead!
            ...
            #If the both left and right gives the same distance measurements.:
            ...
            # If both left and right side looks blocked!
            # still want to try to pass the obstacle from one of its sides 
            else:
    ...
    def __move_in_direction(self, direction):
        if direction == self.RIGHT:
            ...
            raise ValueError('Expected "' + self.UP + '" / "' + self.LEFT + '" / "' + self.RIGHT + '", instead got ' + str(direction))
        
        else:
            raise ValueError('Expected variable of type str and got a variable of type ' + type(direction).__name__)
    ...
    def __update_distance(self, direction):
        ...
        #updating the distance travelled in a certain direaction.
        if direction == self.RIGHT:
        ...
        #Updata last known location attributes 
    ...
    def __safe_for_landing(self):
        ...
        print("ARdupilot height: " + str(drone_altitude) + ", Sensor height: " + str(bottom_sensor_distance))
    ...
    # follwing the safety protocols for the drone:
